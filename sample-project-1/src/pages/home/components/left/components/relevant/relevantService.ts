const relevantList = [
   {
      id: 1,
      name: "ایلان ماسک",
      headline: "بنیانگذار و رئیس شرکت تسلا",
      img: "https://expatguideturkey.com/wp-content/uploads/2022/04/elon-musk-twitter-.jpg",
      follow: false,
   },
   {
      id: 2,
      name: "وارن بافت",
      headline: "معامله گر سهام و سرمایه گذار",
      img: "https://img-cdn.inc.com/image/upload/w_1920,h_1080,c_fill/images/panoramic/GettyImages-492444122_512163_psnryw.jpg",
      follow: false,
   },
   {
      id: 3,
      name: "جف بیزوس",
      headline: "رئیس و بنیانگذار آمازون",
      img: "https://imageio.forbes.com/specials-images/imageserve/5bb22ae84bbe6f67d2e82e05/0x0.jpg?format=jpg&crop=1012,1013,x627,y129,safe&height=416&width=416&fit=bounds",
      follow: false,
   },
   {
      id: 4,
      name: "بیل گیتس",
      headline: "بنیانگذار مایکروسافت",
      img: "https://wp.technologyreview.com/wp-content/uploads/2021/02/20210128_Keatley-Bill_Gates_0107_R.jpeg?w=1500",
      follow: false,
   },
   {
      id: 5,
      name: "ایلان ماسک",
      headline: "بنیانگذار و رئیس شرکت تسلا",
      img: "https://expatguideturkey.com/wp-content/uploads/2022/04/elon-musk-twitter-.jpg",
      follow: false,
   },
   {
      id: 6,
      name: "وارن بافت",
      headline: "معامله گر سهام و سرمایه گذار",
      img: "https://img-cdn.inc.com/image/upload/w_1920,h_1080,c_fill/images/panoramic/GettyImages-492444122_512163_psnryw.jpg",
      follow: false,
   },
   {
      id: 7,
      name: "جف بیزوس",
      headline: "رئیس و بنیانگذار آمازون",
      img: "https://imageio.forbes.com/specials-images/imageserve/5bb22ae84bbe6f67d2e82e05/0x0.jpg?format=jpg&crop=1012,1013,x627,y129,safe&height=416&width=416&fit=bounds",
      follow: false,
   },
   {
      id: 8,
      name: "بیل گیتس",
      headline: "بنیانگذار مایکروسافت",
      img: "https://wp.technologyreview.com/wp-content/uploads/2021/02/20210128_Keatley-Bill_Gates_0107_R.jpeg?w=1500",
      follow: false,
   },
   {
      id: 9,
      name: "ایلان ماسک",
      headline: "بنیانگذار و رئیس شرکت تسلا",
      img: "https://expatguideturkey.com/wp-content/uploads/2022/04/elon-musk-twitter-.jpg",
      follow: false,
   },
   {
      id: 10,
      name: "وارن بافت",
      headline: "معامله گر سهام و سرمایه گذار",
      img: "https://img-cdn.inc.com/image/upload/w_1920,h_1080,c_fill/images/panoramic/GettyImages-492444122_512163_psnryw.jpg",
      follow: false,
   },
   {
      id: 11,
      name: "جف بیزوس",
      headline: "رئیس و بنیانگذار آمازون",
      img: "https://imageio.forbes.com/specials-images/imageserve/5bb22ae84bbe6f67d2e82e05/0x0.jpg?format=jpg&crop=1012,1013,x627,y129,safe&height=416&width=416&fit=bounds",
      follow: false,
   },
   {
      id: 12,
      name: "بیل گیتس",
      headline: "بنیانگذار مایکروسافت",
      img: "https://wp.technologyreview.com/wp-content/uploads/2021/02/20210128_Keatley-Bill_Gates_0107_R.jpeg?w=1500",
      follow: false,
   },
];

export const getRelevantList = () => {
   return relevantList;
};

export type relevantListType = typeof relevantList;
